...
FPanaudiaNodeState FPanaudiaNodeState::FromUnrealCoordinates(const FVector& Position, const FRotator& Rotation, float WorldExtent)
{
    ...
    State.Y = FMath::Clamp(-Position.Y * Scale + 0.5f, 0.0f, 1.0f);  // UE Y (right) -> negate -> Panaudia Y (left)
    State.Z = FMath::Clamp(Position.Z * Scale + 0.5f, 0.0f, 1.0f);
...
{
    // Convert back from Panaudia (X fwd, Y left, Z up) to Unreal (X fwd, Y right, Z up)
    return FVector(
        X * 100.0f,   // X stays X
        -Y * 100.0f,  // Panaudia Y (left) -> negate -> UE Y (right)
        Z * 100.0f    // Z stays Z
    );
}
